...
def train_epoch(model, score_func, data, optimizer, args, device):
    ...
    for perm in d:
        ...
        masked_adj = SparseTensor.from_edge_index(
            edge2keep.t(), sparse_sizes=(data["num_nodes"], data["num_nodes"])
        ).to_device(device)
        ...
        if args.mask_input:
            ...
            masked_adjt = SparseTensor.from_edge_index(
                edge2keep.t(), sparse_sizes=(data["num_nodes"], data["num_nodes"])
            ).to_device(device)
        ...
        neg_edges = torch.randint(
            0,
            data["num_nodes"],
            (edges.size(0), edges.size(1) * args.num_negative),
            dtype=torch.long,
            device=h.device,
        )
...
def train_loop(args, train_args, data, device, loggers, seed, model_save_name, verbose):
    ...
    score_func = mlp_score(
        model.out_dim, model.out_dim, 1, args.pred_layers, train_args["pred_dropout"]
    ).to(device)
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lr_lambda=lambda e: train_args["decay"] ** e
    )
    ...
    for epoch in range(1, 1 + args.epochs):
        print(
            (
                f">>> Epoch {epoch} - {datetime.now().strftime('%H:%M:%S')}\n"
                if verbose
                else ""
            ),
            flush=True,
...
def train_data(args, train_args, data, device, verbose=False):
    ...
    if (
        "citation" in data["dataset"]
        or data["dataset"] in ["cora", "citeseer", "pubmed", "chameleon", "squirrel"]
        or args.heart
    ):
    ...
    return (
        best_mean[1],
        f"{best_mean[1]} ± {best_var[1]}",
        f"{best_mean[2]} ± {best_var[2]}",
    )
