...
def read_data_ogb(args, device):
    ...
    if args.data_name != "ogbl-citation2":
        ...
        data_obj["train_pos"] = torch.cat(
            [source_edge.unsqueeze(1), target_edge.unsqueeze(1)], dim=-1
        ).to(device)
        ...
        data_obj["valid_pos"] = torch.cat(
            [source.unsqueeze(1), target.unsqueeze(1)], dim=-1
        ).to(device)
    ...
    if hasattr(data, "x") and data.x is not None:
        ...
        data_obj["x"] = torch.nn.Parameter(
            torch.zeros(data_obj["num_nodes"], args.dim).to(device)
        )
    ...
    data_obj["adj_t"] = SparseTensor.from_edge_index(
        edge_index, edge_weight.squeeze(-1), [data.num_nodes, data.num_nodes]
    ).to(device)
    ...
    if args.data_name == "ogbl-citation2":
        ...
        data_obj["adj_mask"] = (
            data_obj["adj_t"].to_symmetric().to_torch_sparse_coo_tensor()
        )
    else:
        data_obj["adj_mask"] = (
            data_obj["adj_t"].to_symmetric().to_torch_sparse_coo_tensor()
    ...
    if args.use_val_in_test:
        ...
        data["full_edge_index"] = data_obj["full_edge_index"] = full_edge_index.to(
            device
        )

        val_edge_weight = torch.ones(
            [val_edge_index.size(1), 1], dtype=torch.float, device=device
        )
        val_edge_weight = torch.cat([edge_weight, val_edge_weight], 0).view(-1)
        data_obj["full_adj_t"] = SparseTensor.from_edge_index(
            full_edge_index, val_edge_weight, [data.num_nodes, data.num_nodes]
        ).to(device)
    ...
    data_obj["degree"] = degree(edge_index[0], num_nodes=data_obj["num_nodes"]).to(
        device
    )
    if args.use_val_in_test:
        data_obj["degree_test"] = degree(
            full_edge_index[0], num_nodes=data_obj["num_nodes"]
        ).to(device)

    ### Load PPR matrix
    print("Reading PPR...", flush=True)
    data_obj["ppr"] = get_ppr(
        args.data_name, edge_index, data["num_nodes"], 0.15, args.eps, False
    ).to(device)

    if args.use_val_in_test:
        data_obj["ppr_test"] = get_ppr(
            args.data_name,
...
def read_data_planetoid(args, device):
    ...
    ### Degree of nodes
    data["degree"] = degree(data["edge_index"][0], num_nodes=data["num_nodes"]).to(
        device
    )

    ### Load PPR Matrix
    data["ppr"] = get_ppr(
        args.data_name, data["edge_index"], data["num_nodes"], 0.15, args.eps, False
    ).to(device)
